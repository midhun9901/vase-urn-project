...
story += [
    Spacer(1, 0.2 * cm),
    P("Abstract", absth),
    P("This report describes a content-based image retrieval system for archaeological vases and urns, built around metric learning. Given a query image, the system ranks a database by visual similarity in a learned embedding space. I compare two feature backbones, ResNet50 and DINOv2, together with several metric-learning losses, and I test two ideas suggested by the project material: removing the background with the Segment Anything Model, and describing each image by the pose of its painted figures. The strongest method, DINOv2 features adapted with a triplet-loss projection, reaches 83.39% mean Average Precision and retrieves a correct match within the top ten for every test query. Background removal and pose are both shown to underperform, with reasons grounded in the dataset. Following an initial preparation phase, the work was carried out as a series of controlled, versioned experiments that share one pipeline and one evaluator and run reproducibly on the university HPC cluster.", abst),
    Spacer(1, 0.3 * cm),
]

# ---- 1 introduction ----------------------------------------------------------
story += [heading("1", "Introduction"),
P("This project deals with content-based image retrieval for archaeological vases and urns. The practical question behind it is simple to state: given a photograph or drawing of a vase, find other images that show the same object. This is useful for art-historical research, where the same vessel often appears in several catalogues, museum records, and scholarly illustrations, but under different lighting, framing, and reproduction quality, so a keyword search is rarely enough to connect them."),
...
P("The objectives were to build a reproducible retrieval pipeline that turns vase images into embeddings and evaluates ranking quality with standard metrics; to compare feature backbones and loss functions in order to find a representation that works well on a small dataset with few images per object; to test two ideas suggested by the project material, namely isolating the vase from its background and using the pose of the painted figures as a similarity signal; and to run everything on the university HPC cluster in a way that can be repeated from a single job script."),
...
P("Following the controlled approach set out in Section 3, the experiments are numbered as versions. Each one changes a single design decision relative to the line of work it follows, which is what lets the results in Section 7 be read as a sequence of cause and effect rather than a set of unrelated runs."),
...
P("<b>Metric learning is necessary, and the backbone sets the ceiling.</b> The jump from V1 (15.51%) to V2 (54.79%) shows that raw ImageNet features are not suited to this task and that even a small trained projection helps a great deal. The jump from V6a (32.53%) to V6b (83.39%) shows the same effect for DINOv2: the untrained features are already better than raw ResNet50 but only become strong once they are adapted with triplet loss. At the same time, ResNet50 with triplet loss reaches the mid-fifties while DINOv2 with the same loss reaches the low-eighties, so the representation matters more than the loss."),
P("<b>The loss function is a second-order effect here.</b> V3 (ProxyAnchor, 57.43%) is only slightly above V2 (Triplet, 54.79%), and both share the same Accuracy@1 and Accuracy@10. On this dataset the choice between these two losses is minor compared with the choice of backbone."),
P("<b>Background removal hurts.</b> This was the clearest negative result. SAM cropping collapses performance to 11.55% mAP in both V4 and V5, below even the raw baseline. Because V4 and V5 use different losses and give identical numbers, the loss is not the cause; the cropping is. The global shape and surrounding context of the vessel carry much of the discriminative signal, and cutting the object out of its frame throws that away. After this I stopped preprocessing the input and focused on the representation, which led to DINOv2."),
P("<b>Pose is a genuinely weaker signal, even done correctly.</b> The first pose attempt (V7, 39.76%) used a generic human-pose model. Inspecting its outputs showed the problem: the model is trained on real human bodies, so on most vase images it detects nothing and returns an all-zero vector. The honest fix was to train a pose model on the vase figures themselves. The fine-tuned YOLOv11 model trained well, with box mAP@50 around 0.92 and pose mAP@50 around 0.75 on validation, and it produced a real detection on 197 of 198 training images, about 99%, compared with mostly empty vectors before. With proper features the retrieval rose to 45.55% mAP (Figure 3). The key point is that even with a correct model that detects a figure on almost every image, pose retrieval reaches only about half of what DINOv2 achieves, because keypoints describe the posture of the painted figure rather than the vessel as a whole."),
img(f3, 11.0, "Figure 3. Generic versus vase-specific pose. Fine-tuning lifts every metric, but pose retrieval still trails the appearance-based method by a wide margin."),
P("<b>A note on evaluation.</b> Earlier in the project the retrieval used a fixed top-k of ten, which meant that Accuracy@10 effectively had only nine non-self results to work with. Switching to a full ranking and removing the query from its own list fixed this, and all reported numbers use the corrected evaluator.")]
...
P("The project produced a working, reproducible retrieval system for vases and urns and a clear comparison of design choices. The strongest method, DINOv2 features adapted with a triplet-loss MLP, reaches 83.39% mAP and retrieves a correct match within the top ten for every query in the test set. Two intuitive ideas were tested and shown not to help: removing the background with SAM is actively harmful because it discards global context, and figure-pose keypoints, even from a model trained on the vases and detecting figures on almost every image, give only about half the retrieval quality of the appearance-based approach."),
P("Several directions remain open. The dataset is small, so collecting more images per object would likely raise the ceiling and make the comparison between losses more meaningful. A larger DINOv2 variant, or light fine-tuning of the backbone rather than only training a projection on top, is the natural next step for the appearance branch. Pose did not work as a standalone signal, but it has not been tested as a complement to appearance; combining the DINOv2 embedding with the pose embedding, possibly after first cropping to the detected image field, is the one pose idea still worth trying. Finally, validating the system against an external catalogue would give a stronger measure of real-world usefulness.")]
