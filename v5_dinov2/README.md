...
It writes all outputs into `runs/` so it does not overwrite the existing ResNet, SAM, or Triplet artifacts.

## Experiments

### V5a - DINOv2 Full-Image Retrieval
...
```bash
python pipeline_dinov2/extract_features.py --output-dir runs/dinov2_full
...
### V5b - DINOv2 + Triplet Loss

```text
image -> DINOv2 feature -> MLP 384/768 -> 512 -> 128 -> Triplet Loss -> FAISS full ranking -> evaluate
```

Run:

```bash
python pipeline_dinov2/extract_features.py --output-dir runs/dinov2_triplet
